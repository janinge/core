"""The config flow tests for the owntone media player platform."""
...
async def test_config_flow(hass, config_entry):
    ...
    with patch(
        "homeassistant.components.owntone.config_flow.ForkedDaapdAPI.test_connection",
        new=AsyncMock(),
    ) as mock_test_connection, patch(
        "homeassistant.components.owntone.media_player.ForkedDaapdAPI.get_request",
        autospec=True,
    ) as mock_get_request:
...
async def test_config_flow_zeroconf_invalid(hass):
    ...
    assert result["type"] == data_entry_flow.RESULT_TYPE_ABORT
    assert result["reason"] == "not_owntone"
