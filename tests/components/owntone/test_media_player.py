"""The media player tests for the owntone media player platform."""

from unittest.mock import patch

import pytest

from homeassistant.components.owntone.const import (
...
TEST_ZONE_ENTITY_NAMES = [
    "media_player.owntone_output_" + x
...
async def mock_api_object_fixture(hass, config_entry, get_request_return_values):
    ...
    with patch(
        "homeassistant.components.owntone.media_player.ForkedDaapdAPI",
        autospec=True,
    ) as mock_api:
...
async def test_async_play_media_tts_timeout(hass, mock_api_object):
    ...
    with patch("homeassistant.components.owntone.media_player.TTS_TIMEOUT", 0):
        initial_state = hass.states.get(TEST_MASTER_ENTITY_NAME)
        await _service_call(
            hass,
            TEST_MASTER_ENTITY_NAME,
...
async def pipe_control_api_object_fixture(
    ...
    with patch(
        "homeassistant.components.owntone.media_player.LibrespotJavaAPI",
        autospec=True,
    ) as pipe_control_api:
...
async def test_librespot_java_play_media_pause_timeout(hass, pipe_control_api_object):
    ...
    with patch(
        "homeassistant.components.owntone.media_player.CALLBACK_TIMEOUT", 0
    ):
        initial_state = hass.states.get(TEST_MASTER_ENTITY_NAME)
        await _service_call(
            hass,
